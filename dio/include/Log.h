...
public:
	...
	static void err(const char* fmt, ... ){ FMTPARCE syslog((int)LogPriority::Err, "%s", buffer);};
	static void warn(const char* fmt, ... ){ FMTPARCE syslog((int)LogPriority::Warning, "%s", buffer);};
	static void info(const char* fmt, ... ){ FMTPARCE syslog((int)LogPriority::Info, "%s", buffer);};
